        ...
        <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-600">
              Lane {alert.lane} · rank {alert.rank} · blended score {alert.blended_score.toFixed(2)}
            </p>
          </div>
          <div className="grid gap-3 sm:grid-cols-3">
            ...
            <div className="rounded-2xl bg-violet-50 p-4">
              <div className="text-xs uppercase tracking-[0.2em] text-violet-700/80">Lane</div>
              <div className="mt-2 text-lg font-semibold text-violet-700">{alert.lane}</div>
            </div>
            ...
            <tbody>
              {alert.feature_snapshot.map((f, i) => (
                <tr key={i} className="border-t border-slate-100 transition hover:bg-slate-50/80">
                  <td className="px-6 py-4 font-medium text-slate-950">{f.feature_name}</td>
          ...
          <div className="soft-panel rounded-[24px] p-5">
            ...
            <div className="mt-4 space-y-3 text-sm">
              <div className="flex items-center justify-between gap-3 rounded-2xl bg-slate-50 px-4 py-3">
                <span className="text-slate-600">Primary detector</span>
                <span className="font-medium text-slate-950">
                  {alert.triggering_detectors[0]?.detector ?? "—"}
                </span>
              </div>
              <div className="flex items-center justify-between gap-3 rounded-2xl bg-slate-50 px-4 py-3">
